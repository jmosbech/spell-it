...
module.exports = {
	...
	groupSeparator: '-',
	space: '-',
	postTransform: function(str, amount) {
		if (isMultipleOfHundred(amount) && hasMoreThanOneCent(amount)) {
...
function isMultipleOfHundred(amount) {
	return amount % 100 === 0;
}

function hasMoreThanOneCent(amount) {
	return (amount / 100) % 10 > 1;
}
