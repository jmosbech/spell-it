...
describe('spell-it in Dutch', function(){
	...
	it('should return "één" when input is 1', function(){
		var output = spell(1);
		expect(output).to.equal('één');
	});
	...
	it('should combine compound numbers from twenty-one through ninety-nine', function(){
		var output = spell(21);
		expect(output).to.equal('eenentwintig');
	});
	...
	it('should return "één miljoen tweehonderdduizend één" if input is 1200002', function(){
		var output = spell(1200002);
		expect(output).to.equal('één miljoen tweehonderdduizend twee');
	});

	it('should return "één miljard vijf miljoen tweehonderdduizend één" if input is 1005200002', function(){
		var output = spell(1005200002);
		expect(output).to.equal('één miljard vijf miljoen tweehonderdduizend twee');
	});

	it('should return "vijf miljard vijf miljoen tweehonderdduizend één" if input is 5005200002', function(){
		var output = spell(5005200002);
		expect(output).to.equal('vijf miljard vijf miljoen tweehonderdduizend twee');
	});
});
